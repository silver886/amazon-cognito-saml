...
export async function callback(
   ...
   try {
      const signedCookies = request.signedCookies as Record<
         string,
         string | false
      >;
      ...
      const {id_token: userIdToken} = await (
         await getOidcClient()
      ).callback(
         OIDC_CLIENT_CALLBACK_URL,
         (await getOidcClient()).callbackParams(request),
         {
      ...
      response.setHeader(
         ...
         `script-src 'unsafe-inline'; form-action ${ssoService.location};`,
      );
