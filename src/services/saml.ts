...
export async function sso(
   ...
   try {
      ...
      if (!idpNameIdFormat) {
         throw new ErrorContext(
            new Error('Name ID format of identity provider is empty'),
            {
      ...
      const loginInfo = await idp.parseLoginRequest(
         sp,
         props.binding,
         express.request,
      );
      const {id: spRequestId, assertionConsumerServiceUrl: spRequestAcsUrl} = (
         loginInfo.extract as {
            request: {
               id: string;
      ...
      if (!spAcs) {
         throw new ErrorContext(
            new Error(
               'Assertion consumer service of service provider is empty',
            ),
            {
               source: `[sso] (${__filename})`,
               httpStatusCode: StatusCodes.INTERNAL_SERVER_ERROR,
      ...
      const {Binding: spServiceBinding, Location: spServiceLocation} =
      ...
      express.response.location(
         (await getOidcClient()).authorizationUrl({
