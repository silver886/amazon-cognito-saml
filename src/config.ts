...
import {Constants, IdentityProviderInstance, SamlLib, setSchemaValidator} from 'samlify';
...
export const COOKIE_SECRET = (env.COOKIE_SECRET ?? '') || 'this-is-not-a-safe-secret-to-use';
...
export async function getOidcClient(): Promise<BaseClient> {
    ...
    AWS_COGNITO_CLIENT = new (await Issuer.
        discover(`https://cognito-idp.${AWS_COGNITO_REGION}.amazonaws.com/${AWS_COGNITO_USER_POOL_ID}/.well-known/openid-configuration`)).
        Client({
            /* eslint-disable @typescript-eslint/naming-convention */
            client_id:      AWS_COGNITO_CLIENT_ID,
            client_secret:  AWS_COGNITO_CLIENT_SECRET,
            redirect_uris:  [OIDC_CLIENT_CALLBACK_URL],
            response_types: ['code'],
...
const {
    SAML_IDENTITY_PROVIDER_CERTIFICATE,
    SAML_IDENTITY_PROVIDER_PRIVATE_KEY,
} = env;
...
export const SAML_IDENTITY_PROVIDERS: Record<string, IdentityProviderInstance> = Object.fromEntries(Object.
    keys(SAML_SERVICE_PROVIDERS).
    map((v) => [v, new IdentityProviderInstance({
        ...SAML_IDENTITY_PROVIDER_SETTINGS,
        /* eslint-disable @typescript-eslint/naming-convention */
        entityID:            `${SAML_IDENTITY_PROVIDER_SETTINGS.entityID ?? ''}?spId=${v}`,
        singleSignOnService: SAML_IDENTITY_PROVIDER_SETTINGS.singleSignOnService?.map((w) => ({
            ...w,
