...
export class SamlController extends Controller {
    private static check(caller: string, request: CompositeRequest, spId: string): {
    ...
    public getMetadata(
        ...
        try {
            const {response, serviceProviderId} = SamlController.check('metadata', request, spId);
    ...
    public async getSso(
        ...
        try {
            const {response, serviceProviderId} = SamlController.check('getSso', request, spId);
            const relayState = request.query[Constants.wording.urlParams.relayState];

            return await sso(this, {
                request,
                response,
            }, {
    ...
    public async postSso(
        ...
        try {
            const {response, serviceProviderId} = SamlController.check('postSso', request, spId);
            const relayState = request.query[Constants.wording.urlParams.relayState];
